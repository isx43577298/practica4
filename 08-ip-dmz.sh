...
iptables -A INPUT -s 172.18.0.0/16 -p tcp --dport 22 -j ACCEPT # -i br-xxx
iptables -A INPUT -s 172.18.0.0/16 -p tcp --dport 13 -j ACCEPT # -i br-xxx
iptables -A INPUT -s 172.18.0.0/16 -j REJECT  # -i br-xxx
...
## (3) de la xarxaA només es pot accedir serveis que ofereix la DMZ al servei web
...
# Aquests dos primers exercicis no funcionen perquè el tràfic està tallat a la regla **1**
# caldria rectificar les regles **1** o escriure abans una regla que permeti el tràfic exterior al port
...
# torna a passar el mateix que l'exercici anterior, les regles de l'exercici (2) marcades **1**
# tanquen el trafic forward del port 22 de exterior a xarxaA. Cal obrir-les abans **3**
...
# (6) s'habilita el port 4000 per accedir al port ssh del router/firewal si la ip origen és del hostA1.
iptables -t nat -A PREROUTING -i enp4s0 -p tcp --dport 4000 -s 172.19.0.2 -j DNAT --to :22

## (7) els hosts de la xarxaB tenen accés a tot arreu excepte a la xarxaA.
iptables -A FORWARD -s 172.19.0.0/16 -d 172.18.0.0/16 -p tcp -j DROP
iptables -A FORWARD -s 172.19.0.0/16 -p tcp -j ACCEPT
iptables -A FORWARD -d 172.19.0.0/16 -p tcp -j ACCEPT
